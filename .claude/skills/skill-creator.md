...
This skill creates new skills automatically based on requirements, enabling self-expanding reusable intelligence.

### 1. Parse Skill Requirements

Extract from user input:
- Skill name (lowercase-with-dashes format)
- Skill purpose (one-line description)
- Input parameters
- Output/deliverables
- Success criteria
- Integration points (when to use)

**Example input:**
```text
Create skill for deploying to Kubernetes using Helm charts
```

### 2. Analyze Existing Skills for Patterns

Read all existing skills in `.claude/commands/sp.*.md` to understand:
- Common structure (YAML frontmatter, User Input, Outline)
- Pattern: description, steps, file generation, tests, summary
- Integration patterns (used after X, before Y)
- Success criteria format
- Terminal output format

### 3. Generate Skill Template

**File: `.claude/commands/sp.[skill-name].md`**

```markdown
---
description: [Generated description based on purpose] (project)
---
